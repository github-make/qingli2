...
int CFileTabbedView::OnCreate(const LPCREATESTRUCT lpCreateStruct)
{
    ...
    m_fileAISafetyViewIndex = AddView(RUNTIME_CLASS(CFileAISafetyView), L"AI Safety", CHAR_MAX);
    m_fileAISafetyView = DYNAMIC_DOWNCAST(CFileAISafetyView, GetTabControl().GetTabWnd(m_fileAISafetyViewIndex));
...
void CFileTabbedView::OnInitialUpdate()
{
    ...
    SetAISafetyTabVisibility(false);
...
void CFileTabbedView::SetAISafetyTabVisibility(const bool show)
{
    GetTabControl().ShowTab(m_fileAISafetyViewIndex, show);
}
...
bool CFileTabbedView::CycleTab(const bool forward)
{
    ...
    for (const int tabIndex : { m_fileTreeViewIndex, m_fileTopViewIndex, m_fileDupeViewIndex, m_fileSearchViewIndex, m_fileWatcherViewIndex, m_fileAISafetyViewIndex })
    {
