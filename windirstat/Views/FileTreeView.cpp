...
IMPLEMENT_DYNCREATE(CFileAISafetyView, CControlView)

BEGIN_MESSAGE_MAP(CFileAISafetyView, CControlView)
    ON_WM_CREATE()
END_MESSAGE_MAP()

int CFileAISafetyView::OnCreate(const LPCREATESTRUCT lpCreateStruct)
{
    if (CControlView::OnCreate(lpCreateStruct) == -1) return -1;
    ...
    // Columns should be in enumeration order so initial sort will work
    InsertCol(IDS_COL_NAME, LVCFMT_LEFT, 400, COL_AISAFETY_NAME);
    m_control.InsertColumn(CHAR_MAX, L"Safety Level", LVCFMT_LEFT, DpiRest(80), COL_AISAFETY_LEVEL);
    InsertCol(IDS_COL_SIZE_PHYSICAL, LVCFMT_RIGHT, 90, COL_AISAFETY_SIZE);
    m_control.InsertColumn(CHAR_MAX, L"Reason", LVCFMT_LEFT, DpiRest(300), COL_AISAFETY_REASON);
    m_control.SetSorting(COL_AISAFETY_LEVEL, true);
