...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
    ...
    ON_COMMAND(ID_AI_SAFETY_CHECK, &CMainFrame::OnAISafetyCheck)
    ON_UPDATE_COMMAND_UI(ID_AI_SAFETY_CHECK, &CMainFrame::OnUpdateAISafetyCheck)
END_MESSAGE_MAP()
...
void CMainFrame::MoveFocus(const LOGICAL_FOCUS logicalFocus)
{
    switch (logicalFocus)
    {
        ...
        case LF_AILIST: GetFileAISafetyView()->SetFocus(); break;
...
void CMainFrame::OnAISafetyCheck()
{
    // Check if API key is configured
    if (COptions::AIApiKey.Obj().empty())
    {
        AfxMessageBox(L"Please configure your AI API Key in Settings > AI Assistant first.", MB_ICONWARNING);
        return;
    }
    ...
    // Start analysis
    auto* analyzer = CAISafetyAnalyzer::Get();
    analyzer->SetResultCallback([](CItem* item, SAFETY_LEVEL level, const std::wstring& reason)
    {
        if (auto* ctrl = CFileAISafetyControl::Get())
        {
            ctrl->UpdateResult(item, level, reason);
        }
    });
